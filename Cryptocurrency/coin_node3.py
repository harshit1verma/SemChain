<<<<<<< HEAD
#We copied the code from the coin_node2.py file and made changed the port to 5003
...
class Blockchain:
    ...
    def replace_chain(self):
        ...
        for node in network:                                                               #We will iterate over all the nodes in the network
            response = requests.get(f'http://{node}/get_chain')                            #We will send a get request to the node to get the chain we have stored (parsed_url.netloc)
...
def mine_block():                                           
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Lakshaya', amount = 5)
...
app.run(host = '0.0.0.0', port = 5003)                               #We will run the app on port 5000, we will use
